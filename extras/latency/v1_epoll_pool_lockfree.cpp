/**
 * Variant 1: epoll + memory pool + lock-free SPSC queue (baseline).
 * Build: g++ -std=c++20 -O2 -o v1_epoll_pool_lockfree
 * v1_epoll_pool_lockfree.cpp -lpthread
 */
#include "common.hpp"
#include <arpa/inet.h>
#include <chrono>
...
// Pre-allocated message storage and single-producer/single-consumer queue used
// to decouple network I/O from latency accounting without dynamic allocation.
static MemoryPool<Message, POOL_SIZE> g_pool;
static SPSCQueue<Message, QUEUE_SIZE> g_queue;

// Cross-thread flags and aggregated latency statistics. These are updated by
// the consumer thread and sampled by the stats thread.
...
void consumer_thread() {
  // Busy-poll the SPSC queue and compute end-to-end latency for each
  // Message, using the receive timestamp captured in network_thread.
...
void network_thread() {
  // Single-threaded edge-triggered epoll() loop: accepts client connections,
  // reads inbound TCP payloads, wraps each read in a fixed-size Message, and
  // hands it off to the consumer via the SPSC queue.
...
int main() {
  std::cout << "VARIANT=epoll_pool_lockfree\n";
