/**
 * Variant 2: epoll + malloc/free (no pool) + lock-free SPSC queue.
 * Build: g++ -std=c++20 -O2 -o v2_epoll_nopool_lockfree
 * v2_epoll_nopool_lockfree.cpp -lpthread
 */
#include "common.hpp"
#include <arpa/inet.h>
#include <chrono>
...
// Lock-free SPSC queue shared between the epoll loop and the consumer thread.
static SPSCQueue<Message, QUEUE_SIZE> g_queue;

// Same latency accounting pattern as v1, but this variant allocates Message
// objects with new/delete instead of using a fixed-size memory pool.
...
void consumer_thread() {
  // Pop dynamically-allocated Message objects, compute latency, then delete.
...
int main() {
  std::cout << "VARIANT=epoll_nopool_lockfree\n";
