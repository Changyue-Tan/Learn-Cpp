/**
 * Variant 5: poll() (instead of epoll) + memory pool + lock-free SPSC queue.
 * Build: g++ -std=c++20 -O2 -o v5_poll_pool_lockfree v5_poll_pool_lockfree.cpp
 * -lpthread
 */
#include "common.hpp"
#include <algorithm>
#include <arpa/inet.h>
#include <chrono>
#include <fcntl.h>
#include <iostream>
#include <netinet/tcp.h>
#include <poll.h>
#include <sys/socket.h>
#include <thread>
#include <unistd.h>

constexpr size_t POOL_SIZE = 1024;
constexpr size_t QUEUE_SIZE = 512;
constexpr uint16_t PORT = 9999;
constexpr int MAX_FDS =
    64; // max concurrent connections (pfds[0]=listen, rest=clients)

// Fixed-size message pool and SPSC queue, same as variant 1, but the network
// side uses poll() instead of epoll() to drive readiness.
static MemoryPool<Message, POOL_SIZE> g_pool;
static SPSCQueue<Message, QUEUE_SIZE> g_queue;

// Shared lifetime flag and latency statistics across network/consumer/stats.
static std::atomic<bool> g_running{true};
static std::atomic<int64_t> g_total_ns{0};
static std::atomic<uint64_t> g_count{0};
static std::atomic<int64_t> g_max_ns{0};

static void set_nonblocking(int fd) {
  int flags = fcntl(fd, F_GETFL, 0);
  fcntl(fd, F_SETFL, flags | O_NONBLOCK);
}
static void set_tcp_nodelay(int fd) {
  // Disable Nagle; reduces latency for small messages.
  int yes = 1;
  setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, &yes, sizeof(yes));
}

void consumer_thread() {
  // Identical accounting logic to the other variants so differences in the
  // plots primarily reflect the readiness primitive (poll vs epoll) and
  // connection-handling strategy.
  while (g_running.load(std::memory_order_relaxed)) {
    Message *msg = g_queue.pop();
    if (!msg) {
      CPU_PAUSE();
      continue;
    }
    int64_t latency = now_ns() - msg->recv_ns;
    g_total_ns.fetch_add(latency, std::memory_order_relaxed);
    g_count.fetch_add(1, std::memory_order_relaxed);
    // Lock-free max update: CAS until we win or someone else has a larger
    // value.
    int64_t prev = g_max_ns.load(std::memory_order_relaxed);
    while (latency > prev && !g_max_ns.compare_exchange_weak(
                                 prev, latency, std::memory_order_relaxed)) {
    }
    if (msg->len >= 4) {
      uint32_t price;
      std::memcpy(&price, msg->data, sizeof(price));
      (void)price;
    }
    g_pool.free(msg);
  }
}

void network_thread() {
  // Setup listen socket (same as epoll variants).
  int listen_fd = socket(AF_INET, SOCK_STREAM, 0);
  int opt = 1;
  setsockopt(listen_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
  set_nonblocking(listen_fd);
  sockaddr_in addr{};
  addr.sin_family = AF_INET;
  addr.sin_addr.s_addr = INADDR_ANY;
  addr.sin_port = htons(PORT);
  if (bind(listen_fd, (sockaddr *)&addr, sizeof(addr)) < 0)
    throw std::runtime_error("bind failed");
  if (listen(listen_fd, 10) < 0)
    throw std::runtime_error("listen failed");

  // poll() uses a flat array: pfds[0]=listen, pfds[1..nfds-1]=client conns.
  pollfd pfds[MAX_FDS];
  int nfds = 1;
  pfds[0].fd = listen_fd;
  pfds[0].events = POLLIN;
  uint32_t seq = 0;

  while (g_running.load(std::memory_order_relaxed)) {
    int ret =
        poll(pfds, nfds, 100); // 100ms timeout to periodically check g_running
    if (ret <= 0)
      continue;
    for (int i = 0; i < nfds && ret > 0; ++i) {
      if (!(pfds[i].revents & POLLIN))
        continue;
      ret--;
      int fd = pfds[i].fd;
      if (fd == listen_fd) {
        // New connection: accept and add to pfds if under MAX_FDS.
        sockaddr_in peer{};
        socklen_t plen = sizeof(peer);
        int conn = accept4(listen_fd, (sockaddr *)&peer, &plen, SOCK_NONBLOCK);
        if (conn < 0 || nfds >= MAX_FDS) {
          if (conn >= 0)
            close(conn);
          continue;
        }
        set_tcp_nodelay(conn);
        pfds[nfds].fd = conn;
        pfds[nfds].events = POLLIN;
        nfds++;
        continue;
      }
      // Client fd: read until EAGAIN (nonblock) or EOF.
      char buf[1024];
      ssize_t bytes;
      while ((bytes = recv(fd, buf, sizeof(buf), 0)) > 0) {
        int64_t ts = now_ns();
        Message *msg = g_pool.alloc();
        if (!msg)
          continue;
        msg->recv_ns = ts;
        msg->seq = seq++;
        msg->len =
            static_cast<uint16_t>(std::min((ssize_t)sizeof(msg->data), bytes));
        std::memcpy(msg->data, buf, msg->len);
        while (!g_queue.push(msg))
          CPU_PAUSE();
      }
      if (bytes == 0) {
        // Client closed; remove from pfds by swapping with last, then
        // decrement.
        close(fd);
        pfds[i] = pfds[nfds - 1];
        nfds--;
        i--; // Re-check slot i (now holds the former last fd).
      }
    }
  }
  for (int i = 1; i < nfds; ++i)
    close(pfds[i].fd);
  close(listen_fd);
}

void stats_thread() {
  // Print running stats every 2s; main() prints FINAL_STATS on shutdown.
  while (g_running.load(std::memory_order_relaxed)) {
    std::this_thread::sleep_for(std::chrono::seconds(2));
    uint64_t cnt = g_count.load(std::memory_order_relaxed);
    if (cnt > 0)
      std::cout << "[STATS] count=" << cnt << " avg_ns="
                << (g_total_ns.load(std::memory_order_relaxed) / (int64_t)cnt)
                << " max_ns=" << g_max_ns.load(std::memory_order_relaxed)
                << "\n";
  }
}

int main() {
  std::cout << "VARIANT=poll_pool_lockfree\n";
  std::thread t_net(network_thread), t_con(consumer_thread),
      t_sta(stats_thread);
  std::cin
      .get(); // Block until Enter; stress_tiers.py sends newline to shut down.
  g_running.store(false, std::memory_order_relaxed);
  t_net.join();
  t_con.join();
  t_sta.join();
  uint64_t cnt = g_count.load();
  if (cnt > 0)
    std::cout << "FINAL_STATS count=" << cnt
              << " avg_ns=" << (g_total_ns.load() / (int64_t)cnt)
              << " max_ns=" << g_max_ns.load() << "\n";
  return 0;
}
