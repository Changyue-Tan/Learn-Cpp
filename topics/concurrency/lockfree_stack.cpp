...
int main() {
    ...
    // Benchmark: push all then drain (single-threaded; compares Treiber vs mutex).
    const int bench_ops = 500'000;

    {
        MutexStack<int> ms;
        auto t0 = std::chrono::steady_clock::now();
        for (int i = 0; i < bench_ops; ++i) ms.push(i);
        int cnt = 0;
        while (ms.pop(v)) ++cnt;
        auto t1 = std::chrono::steady_clock::now();
        auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
        std::println("Benchmark [mutex_stack]: {} push+pop in {} us = {:.0f} ops/sec", bench_ops, us,
    ...
    auto t1 = std::chrono::steady_clock::now();
    auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
    std::println("Benchmark [lockfree_stack]: {} push+pop in {} us = {:.0f} ops/sec", bench_ops, us,
                 1e6 * bench_ops / std::max(1.0, static_cast<double>(us)));
    return 0;
}
