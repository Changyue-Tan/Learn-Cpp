...
int main() {
...

    {
        ...
        std::println("Benchmark [mutex]: {} reads in {} us = {:.0f} read/sec", bench_ops, us_r,
                     1e6 * bench_ops / std::max(1.0, static_cast<double>(us_r)));
        std::println("Benchmark [mutex]: {} writes in {} us = {:.0f} write/sec", bench_ops / 10, us_w,
                     1e6 * (bench_ops / 10) / std::max(1.0, static_cast<double>(us_w)));
    }
    ...
    auto us_r = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
    auto us_w = std::chrono::duration_cast<std::chrono::microseconds>(t2 - t1).count();
    std::println("Benchmark [rwlock]: {} reads in {} us = {:.0f} read/sec", bench_ops, us_r,
                 1e6 * bench_ops / std::max(1.0, static_cast<double>(us_r)));
    std::println("Benchmark [rwlock]: {} writes in {} us = {:.0f} write/sec", bench_ops / 10, us_w,
                 1e6 * (bench_ops / 10) / std::max(1.0, static_cast<double>(us_w)));
    (void)val;
    return 0;
}
