...
int main() {
...

    {
        ...
        for (int i = 0; i < bench_tasks; ++i) sink += i;
        auto t1 = std::chrono::steady_clock::now();
        auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
        (void)sink;
        std::println("Benchmark [sequential]: {} trivial tasks in {} us = {:.0f} tasks/sec", bench_tasks, us,
                     1e6 * bench_tasks / std::max(1.0, static_cast<double>(us)));
    }
    ...
    auto t1 = std::chrono::steady_clock::now();
    auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
    std::println("Benchmark [thread_pool]: {} submit+get in {} us = {:.0f} tasks/sec", bench_tasks, us,
                 1e6 * bench_tasks / std::max(1.0, static_cast<double>(us)));
    return 0;
}
