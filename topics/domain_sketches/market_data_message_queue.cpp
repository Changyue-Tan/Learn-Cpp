...
int main() {
...

    {
        ...
        std::thread cm([&] {
            Msg m;
            while (!done_m || recv_m.load() < bench_ops) {
                if (mq.consume(m)) ++recv_m;
        ...
        std::println("Benchmark [mutex_deque]: {} publish+consume in {} us = {:.0f} msg/sec", bench_ops, us,
    ...
    auto t1 = std::chrono::steady_clock::now();
    auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
    std::println("Benchmark [ring_padded]: {} publish+consume in {} us = {:.0f} msg/sec", bench_ops, us,
                 1e6 * bench_ops / std::max(1.0, static_cast<double>(us)));
    return 0;
}
