...
int main() {
    ...
    {
        std::vector<Order> flat;
        flat.reserve(bench_orders);
        auto t0 = std::chrono::steady_clock::now();
        for (int i = 0; i < bench_orders; ++i) {
            flat.push_back(Order(static_cast<OrderId>(i), (i % 2 == 0) ? Side::Buy : Side::Sell, 100 + (i % 10), 100));
        }
        auto t1 = std::chrono::steady_clock::now();
        auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
        std::println("Benchmark [vector_append]: {} orders stored in {} us = {:.0f} orders/sec", bench_orders, us,
                     1e6 * bench_orders / std::max(1.0, static_cast<double>(us)));
    }
    ...
    auto t1 = std::chrono::steady_clock::now();
    auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
    std::println("Benchmark [map_book]: {} add_order in {} us = {:.0f} orders/sec", bench_orders, us,
                 1e6 * bench_orders / std::max(1.0, static_cast<double>(us)));
    return 0;
}
