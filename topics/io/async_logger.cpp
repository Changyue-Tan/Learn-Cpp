...
int main() {
...

    {
        ...
        for (int i = 0; i < bench_ops; ++i) std::print("{}", "[INFO] benchmark line\n");
        auto t1 = std::chrono::steady_clock::now();
        auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
        std::println("Benchmark [sync_print]: {} lines in {} us = {:.0f} log/sec", bench_ops, us,
    ...
    auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
    std::println("Benchmark [async_enqueue]: {} log() in {} us = {:.0f} log/sec", bench_ops, us,
                 1e6 * bench_ops / std::max(1.0, static_cast<double>(us)));
    return 0;
}
