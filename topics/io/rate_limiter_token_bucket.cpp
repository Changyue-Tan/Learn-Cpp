...
int main() {
    ...
    {
        std::atomic<long long> tokens{bench_ops + 1};
        auto t0 = std::chrono::steady_clock::now();
        int ok_count = 0;
        for (int i = 0; i < bench_ops; ++i) {
            if (tokens.fetch_sub(1, std::memory_order_relaxed) > 0) ++ok_count;
        }
        auto t1 = std::chrono::steady_clock::now();
        auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
        std::println("Benchmark [atomic_decrement]: {} checks in {} us = {:.0f} ops/sec ({} accepted)", bench_ops, us,
                     1e6 * bench_ops / std::max(1.0, static_cast<double>(us)), ok_count);
    }
    ...
    auto t1 = std::chrono::steady_clock::now();
    auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
    std::println("Benchmark [token_bucket]: {} try_consume in {} us = {:.0f} ops/sec ({} accepted)", bench_ops, us,
                 1e6 * bench_ops / std::max(1.0, static_cast<double>(us)), ok_count);
    return 0;
}
