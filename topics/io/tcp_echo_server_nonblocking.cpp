...
int main() {
    constexpr auto bench_duration = std::chrono::milliseconds(100);
    {
        ...
        uint64_t spins = 0;
        while (std::chrono::steady_clock::now() - t0 < bench_duration) ++spins;
        auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(
                      std::chrono::steady_clock::now() - t0)
                      .count();
        std::println("Benchmark [cpu_spin]: {} spins in {} ms = {:.0f} spin/sec", spins, ms,
                     1000.0 * static_cast<double>(spins) / std::max(1.0, static_cast<double>(ms)));
    }
    ...
    while (true) {
        ...
        if (clients.empty() && std::chrono::steady_clock::now() - bench_start >= bench_duration) {
            std::println("Benchmark [nonblocking_poll]: {} empty loop iters in {} ms = {:.0f} iter/sec",
