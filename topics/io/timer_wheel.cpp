...
int main() {
...

    {
        ...
        for (int i = 0; i < bench_ticks; ++i) sink += static_cast<uint64_t>(i);
        auto t1 = std::chrono::steady_clock::now();
        auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
        (void)sink;
        std::println("Benchmark [empty_loop]: {} iters in {} us = {:.0f} iters/sec", bench_ticks, us,
                     1e6 * bench_ticks / std::max(1.0, static_cast<double>(us)));
    }
    ...
    auto t1 = std::chrono::steady_clock::now();
    auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
    std::println("Benchmark [timer_wheel]: {} ticks in {} us = {:.0f} ticks/sec", bench_ticks, us,
                 1e6 * bench_ticks / std::max(1.0, static_cast<double>(us)));
    return 0;
}
