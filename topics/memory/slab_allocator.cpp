...
int main() {
...

    {
        ...
        for (int i = 0; i < bench_ops; ++i) ptrsb[i] = std::malloc(32);
        for (int i = 0; i < bench_ops; ++i) std::free(ptrsb[i]);
        auto t1 = std::chrono::steady_clock::now();
        auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
        std::println("Benchmark [malloc_32]: {} alloc+dealloc in {} us = {:.0f} ops/sec", bench_ops, us,
    ...
    auto t1 = std::chrono::steady_clock::now();
    auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
    std::println("Benchmark [slab_32]: {} alloc+dealloc in {} us = {:.0f} ops/sec", bench_ops, us,
                 1e6 * bench_ops / std::max(1.0, static_cast<double>(us)));
    return 0;
}
