...
int main() {
...

    {
        ...
        um.reserve(bench_ops * 2);
        auto t0 = std::chrono::steady_clock::now();
        for (int i = 0; i < bench_ops; ++i) um[i] = i * 2;
        for (int i = 0; i < bench_ops; ++i) (void)um.find(i);
        auto t1 = std::chrono::steady_clock::now();
        auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
        std::println("Benchmark [std_unordered_map]: {} insert + {} find in {} us = {:.0f} ops/sec", bench_ops,
                     bench_ops, us, 1e6 * bench_ops * 2 / std::max(1.0, static_cast<double>(us)));
    }
    ...
    auto t1 = std::chrono::steady_clock::now();
    auto us = std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count();
    std::println("Benchmark [open_addressing]: {} insert + {} find in {} us = {:.0f} ops/sec", bench_ops, bench_ops, us,
                 1e6 * bench_ops * 2 / std::max(1.0, static_cast<double>(us)));
    return 0;
}
