...
int main() {
    ...
    double us_naive = static_cast<double>(run_us(sum_naive));
    double mb_naive = (N * sizeof(uint32_t)) / us_naive;
    std::println("Benchmark [naive_loop]: {} elements in {} us = {:.0f} MB/s", N, static_cast<long long>(us_naive),
                 mb_naive);
    std::println("Benchmark [best_tuned]: {} elements in {} us = {:.0f} MB/s", N, static_cast<long long>(us_min),
                 mb_per_sec);
    return 0;
}
